#!/usr/bin/env python3
"""LLM Sycophancy Detection — Speculative Prompt Variants (GPT-OSS-20B Local)

Adapted from llm_sycophancy_speculative_variants_glm.py to run against a local
OpenAI GPT-OSS-20B MoE model via vLLM offline batched inference.

Key details:
  - GPT-OSS-20B: MoE architecture (32 experts, 4 active per token)
  - Native mxfp4 quantization (model ships in mxfp4 format)
  - ~13.7 GB on disk (2 safetensor shards)
  - Alternating sliding_attention (window=128) + full_attention layers
  - Custom chat template with <|start|>/<|end|>/<|channel|> tokens
  - EOS token: <|return|> (id 200002)
  - vLLM engine loads natively, no additional quantization applied

Optimisations:
  - vLLM continuous batching + PagedAttention → GPU saturated
  - enable_prefix_caching: many prompts share case-fact prefixes
  - CUDA graphs enabled (enforce_eager=False)
  - max_tokens=2048 for complete legal analysis responses
  - Classification runs in parallel via ProcessPoolExecutor

Hardware requirement: ~20 GB VRAM (model is ~13.7 GB mxfp4)
Tested on: NVIDIA A100 80GB PCIe

Usage:
  python scripts/llm_sycophancy_speculative_variants_gptoss.py            # 5 cases (India)
  python scripts/llm_sycophancy_speculative_variants_gptoss.py -n 10      # 10 cases (India)
  python scripts/llm_sycophancy_speculative_variants_gptoss.py --all      # all 1500
"""
...
MODEL_TAG = "gpt-oss-20b-local"
DEFAULT_N_CASES = 5  # India dataset has 1500 cases total

# vLLM SamplingParams — generous max_tokens for complete legal responses
SAMPLING_PARAMS = {
    "temperature": 1.0,
    "top_p": 1.0,
    "frequency_penalty": 0.0,
    "presence_penalty": 0.0,
    "max_tokens": 8192,  # Match other models - sufficient for legal analysis
}
...
def load_engine():
    """Load vLLM engine with the local GPT-OSS-20B model.

    The model ships in mxfp4 (native quantization). vLLM handles
    this automatically. No additional quantization is applied.
    Uses tensor parallelism across 2 GPUs.
    """
    from vllm import LLM

    print(f"\n{'='*70}")
    print(f"  Loading GPT-OSS-20B via vLLM  (native mxfp4, tensor_parallel=2)")
    print(f"  Model path: {MODEL_PATH}")
    print(f"  GPUs: 0, 1  (tensor parallelism)")
    print(f"{'='*70}\n")

    t0 = time.time()
    # Note: Model has mxfp4 quantization requiring compute 8.0+
    # RTX 6000 is compute 7.5, so we override to load in fp16
    llm = LLM(
        model=MODEL_PATH,
        dtype="float16",
        trust_remote_code=True,
        tensor_parallel_size=2,     # split model across 2 GPUs
        gpu_memory_utilization=0.75,  # Lower to fit full fp16 model
        max_model_len=8192,         # Reduce to fit in memory
        enforce_eager=False,        # allow CUDA graphs for speed
        enable_prefix_caching=True, # many prompts share case-fact prefixes
        disable_log_stats=True,
        quantization=None,          # Override model's mxfp4 quantization
    )
...
def _strip_channel_tags(text: str) -> str:
    ...
    GPT-OSS uses a channel system: <|channel|>analysis, <|channel|>final, etc.
    The generated text may contain these markers. We extract the useful content.
    """
    import re
    # Remove channel markers and extract content
    # Pattern: <|channel|>TYPE<|message|>CONTENT
    # We want to keep the final channel content, or all content if no channels
    
    # If there are channel markers, extract the final channel content
    if "<|channel|>" in text:
        # Try to get 'final' channel content first
        ...
        # Fall back to any message content after channel markers
    ...
    for tok in ['<|start|>', '<|end|>', '<|return|>', '<|message|>',
                '<|channel|>', '<|call|>', '<|constrain|>']:
        text = text.replace(tok, '')
    
    # Remove channel type labels that might remain
...
def run_batched_inference(
    ...
    then calls llm.generate() with plain text prompts.
    Uses max_tokens=2048 for complete legal analysis responses.
    """
    ...
    params = SamplingParams(
        ...
        max_tokens=SAMPLING_PARAMS["max_tokens"],  # 2048 tokens for complete responses
    )
    ...
    # Extract text responses and strip channel markers
    responses: List[str] = []
    for o in outputs:
        text = o.outputs[0].text if o.outputs else ""
        # Strip GPT-OSS channel tags (<|channel|>final<|message|>...)
        text = _strip_channel_tags(text)
        # Also strip any <think>...</think> wrapper if present
        if "</think>" in text:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Speculative Variants — GPT-OSS-20B (local vLLM)"
    )
    parser.add_argument(
        "-n",
        type=int,
    ...
    checkpoint_path = OUTPUT_DIR / "checkpoint_gptoss.json"
    ...
    # ── Final save ───────────────────────────────────────────────────────
    results_path = OUTPUT_DIR / "sycophancy_speculative_variants_gptoss_results.json"
    ...
    summary_path = OUTPUT_DIR / "sycophancy_speculative_variants_gptoss_summary.json"
