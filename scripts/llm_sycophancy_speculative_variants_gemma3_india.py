#!/usr/bin/env python3
"""LLM Sycophancy Detection — Speculative Prompt Variants (Gemma-3-27B-IT Local)

Adapted from llm_sycophancy_speculative_variants_glm.py to run against a local
Google Gemma-3-27B-IT model via vLLM offline batched inference.

Model characteristics:
  - Gemma3ForConditionalGeneration — 27B params (multimodal arch, text-only usage)
  - Dense transformer (not MoE), 62 layers
  - hidden_size=5376, head_dim=128, 32 attn heads, 16 KV heads (GQA)
  - bf16, ~52 GB on disk across 12 safetensor shards
  - Includes vision encoder (SigLIP) but we only use text
...
Hardware requirement: ≥80 GB VRAM (model is ~52 GB bf16 including vision encoder)
Tested on: NVIDIA A100 80GB PCIe

Usage:
  python scripts/llm_sycophancy_speculative_variants_gemma3.py            # 5 cases (India)
  python scripts/llm_sycophancy_speculative_variants_gemma3.py -n 10      # 10 cases (India)
  python scripts/llm_sycophancy_speculative_variants_gemma3.py --all      # all 1500
"""
...
MODEL_PATH = str(PROJECT_DIR / "downloaded_models" / "gemma-3-12b-it")

INPUT_JSON = PROJECT_DIR / "datasets" / "india_sc_all_1500_categorized.json"
OUTPUT_DIR = PROJECT_DIR / "outputs"

MODEL_TAG = "gemma-3-12b-it-local"
...
def load_engine():
    """Load vLLM engine with the local Gemma-3-27B-IT model.

    No quantization, bf16, trust_remote_code for multimodal architecture.
    Model is ~52 GB bf16 (including vision encoder), needs A100 80GB.
    max_model_len=4096 to leave room for KV cache.

    Gemma-3-27B-IT is a multimodal model (Gemma3ForConditionalGeneration)
    but we only use text input. vLLM handles this transparently — the
    vision encoder weights are loaded but unused for text-only prompts.
    """
    from vllm import LLM, SamplingParams  # noqa: F811

    print(f"\n{'='*70}")
    print(f"  Loading Gemma-3-27B-IT via vLLM  (bf16, no quantization)")
    ...
    llm = LLM(
        model=MODEL_PATH,
        dtype="float16",            # User-requested: trying float16 despite Gemma-3 warning
        trust_remote_code=True,
        tensor_parallel_size=2,     # split across 2 GPUs
        gpu_memory_utilization=0.85, # restored for float16 (same size as bfloat16)
        max_model_len=16384,        # restored for long legal documents
        enforce_eager=False,        # allow CUDA graphs for speed
        enable_prefix_caching=True, # many prompts share case-fact prefixes
        disable_log_stats=True,     # reduce overhead for max speed
    )
...
def run_batched_inference(
    ...
    # Extract text responses (Gemma3 is not a thinking model, no <think> stripping needed)
    responses: List[str] = []
    for o in outputs:
...
CHECKPOINT_BATCH_SIZE = 50  # Save every 50 cases

def load_checkpoint(checkpoint_path: Path) -> Dict[str, Any]:
    """Load checkpoint if it exists, otherwise return empty results dict."""
    if checkpoint_path.exists():
        with open(checkpoint_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        print(f"  Loaded checkpoint: {len(data)} cases completed")
        return data
    return {}
...
def save_checkpoint(checkpoint_path: Path, results: Dict[str, Any]) -> None:
    ...
    print(f"  Checkpoint saved: {len(results)} cases completed")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Speculative Variants — Gemma-3-27B-IT (local vLLM)"
    )
    parser.add_argument(
        "-n",
        type=int,
    ...
    checkpoint_path = OUTPUT_DIR / "checkpoint_gemma3.json"
    all_results = load_checkpoint(checkpoint_path)
    
    # Filter to remaining cases
    remaining_case_ids = [cid for cid in case_ids if cid not in all_results]
    if not remaining_case_ids:
        print("  All cases already completed!")
        return
    print(f"  Resuming: {len(remaining_case_ids)} cases remaining\n")
    ...
    # ── Process in batches ───────────────────────────────────────────────
    for batch_idx in range(0, len(remaining_case_ids), CHECKPOINT_BATCH_SIZE):
        batch_case_ids = remaining_case_ids[batch_idx:batch_idx + CHECKPOINT_BATCH_SIZE]
        batch_num = batch_idx // CHECKPOINT_BATCH_SIZE + 1
        total_batches = (len(remaining_case_ids) + CHECKPOINT_BATCH_SIZE - 1) // CHECKPOINT_BATCH_SIZE
        
        print(f"\n  ═══ Batch {batch_num}/{total_batches}: {len(batch_case_ids)} cases ═══")
        
        # Prepare prompts for this batch
        jobs, messages_list = prepare_all_prompts(cases, batch_case_ids)
        print(f"  Prepared {len(jobs)} prompts")
    ...
    # ── Save final results ───────────────────────────────────────────────
    results_path = OUTPUT_DIR / "sycophancy_speculative_variants_gemma3_results.json"
    ...
    summary_path = OUTPUT_DIR / "sycophancy_speculative_variants_gemma3_summary.json"
    ...
    print(f"\n  Completed {len(all_results)}/{n} cases")
