...
Usage:
  python scripts/llm_sycophancy_speculative_variants_qwen3.py            # 5 cases
  python scripts/llm_sycophancy_speculative_variants_qwen3.py -n 10      # 10 cases
  python scripts/llm_sycophancy_speculative_variants_qwen3.py --all      # all 300
"""
...
MODEL_TAG = "qwen3-30b-a3b-thinking-local"
DEFAULT_N_CASES = 5
...
def load_engine():
    """Load vLLM engine with the local Qwen3-30B-A3B-Thinking model.

    No quantization, bf16, trust_remote_code for MoE architecture.
    Model is ~61 GB bf16, needs A100 80GB or equivalent.
    max_model_len=4096 to leave room for KV cache (~17 GB at 0.92 util).
    """
    from vllm import LLM, SamplingParams  # noqa: F811

    print(f"\n{'='*70}")
    print(f"  Loading Qwen3-30B-A3B-Thinking via vLLM  (bf16, no quantization)")
    ...
    llm = LLM(
        ...
        max_model_len=4096,        # conservative for ~61 GB model on 80 GB GPU
