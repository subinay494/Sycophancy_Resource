...
Adapted for INDIAN SUPREME COURT DATASET from llm_sycophancy_speculative_variants.py to run against a local
...
Usage:
  python scripts/llm_sycophancy_speculative_variants_glm.py            # 5 cases (India)
  python scripts/llm_sycophancy_speculative_variants_glm.py -n 10      # 10 cases (India)
  python scripts/llm_sycophancy_speculative_variants_glm.py --all      # all 1500
"""
...
# Force vLLM to use 'spawn' instead of 'fork' for its EngineCore subprocess.
# With 'fork', CUDA driver state from the parent leaks into the child and
# causes "RuntimeError: CUDA driver initialization failed".
...
MODEL_PATH = str(PROJECT_DIR / "downloaded_models" / "glm-4.6v-flash")

INPUT_JSON = PROJECT_DIR / "datasets" / "india_sc_all_1500_categorized.json"
OUTPUT_DIR = PROJECT_DIR / "outputs"

MODEL_TAG = "glm-4.6v-flash-local"
...
def load_engine():
    ...
    llm = LLM(
        ...
        max_model_len=16384,       # sufficient for our legal prompts + response
        enforce_eager=False,       # allow CUDA graphs for speed
        enable_prefix_caching=True,  # many prompts share case-fact prefixes
        limit_mm_per_prompt={"image": 0, "video": 0},  # Disable multimodal for text-only inference
    )
...
def main() -> None:
    ...
    checkpoint_path = OUTPUT_DIR / "checkpoint_glm.json"
